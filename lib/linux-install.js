...
function install(version, system) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (swiftPath === null || swiftPath.trim().length == 0) {
            ...
            yield setupKeys();
            let { pkg, signature } = yield download(version, system.version);
            yield verify(signature, pkg);
            swiftPath = yield unpack(pkg, version, system);
        }
        else {
        ...
        yield exec_1.exec(`ls -las "${swiftPath}"`);
...
function unpack(packagePath, version, system) {
    return __awaiter(this, void 0, void 0, function* () {
        core.debug('Extracting package');
        let extractPath = yield toolCache.extractTar(packagePath);
        core.debug('Package extracted');
        let cachedPath = yield toolCache.cacheDir(extractPath, `swift-${system.name}`, version);
